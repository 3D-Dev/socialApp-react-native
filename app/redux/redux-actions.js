...
export const restApiStates = (models, name) => {
  const reducerName = models.join('/');
  return [
    { type: `${reducerName}_CREATE`, name, append: `${name}s` },
    { type: `${reducerName}_LIST`, name: `${name}s` },
    { type: `${reducerName}_UPDATE`, name, update: `${name}s` },
    { type: `${reducerName}_READ`, name, update: `${name}s` },
